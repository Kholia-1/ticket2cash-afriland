...
class CashbackCreditSchemaMigrationTest {
    ...
    void migrationAddsCreditColumnsAndPreservesExistingValues() {
        ...
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS campaign_cashback_amount DECIMAL(18,2)");
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS loyalty_bonus_enabled BOOLEAN");
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS loyalty_tier_name VARCHAR(255)");
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS loyalty_bonus_percent DECIMAL(9,4)");
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS loyalty_bonus_amount DECIMAL(18,2)");
        verify(jdbcTemplate).execute("ALTER TABLE cashback_payments ADD COLUMN IF NOT EXISTS final_cashback_amount DECIMAL(18,2)");
